          ...
          title={
            <span className="inline-flex items-center gap-2">
              ...
              <Link 
                ...
                style={{ '--hover-color': color.toString() } as React.CSSProperties}
              >
                {title}
              </Link>
            </span>
        ...
        >
          <div className="flex flex-col gap-4 justify-center">
            <p className=" text-ctp-text font-bold">
              {startDate.toLocaleDateString()} - {endDate?.toLocaleDateString() ?? t("jobs.now")}
            </p>
            <div className="flex flex-col gap-2">
              <p className=" text-ctp-text font-bold">{t("jobs.done")}</p>
              <ul className="flex flex-col">
                {list?.map((item, index) => (
                  <li key={index} className=" text-ctp-text text-sm flex gap-2 items-center">
                    <div className={`w-[6px] h-[6px]  rounded-full`} style={{backgroundColor: color.toString()}} />
                    <span className="underline">{t(item.text)}</span>{item.emoji} 
                  </li>
                ))}
              </ul>
            </div>
